...
import { DialogConfirmationComponent } from '../dialog-confirmation/dialog-confirmation.component'; // ✅ correct path
...
export class SubscriberFormComponent implements OnInit, OnDestroy {
  ...
  servicesRequired = ['Dynamic', 'Static', 'Chart'];
  subscriberCodes: string[] = [];
  selectedSubscriberId: string = '' ;

  constructor(private fb: FormBuilder, private vendorService: VendorService, private router: Router,  private dialog: MatDialog ) {
  ...
  onSubmitSubscriber() {
    ...
    const dialogRef = this.dialog.open(DialogConfirmationComponent, {
      width: '400px',
      data: {
        subscriber_id: response?.subscriber_id || response?.subscriberId,
        nodal_email: response?.nodal_email || response?.nodalEmail,
        type: 'subscriber'
      }
    });

    // ✅ Navigate only after popup is closed
