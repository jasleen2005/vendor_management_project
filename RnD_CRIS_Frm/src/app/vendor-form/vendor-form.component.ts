...
export class VendorFormComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    // this.vendorService.getDivisions().subscribe(data => {
    // console.log('Divisions:', data);  // 🔍 Check here if all divisions are loaded
    // this.divisions = data;
    // });

    console.log('Selected zone:', this.vendorForm.get('zone')?.value);

  }

  onZoneChange() {
    const selectedZone = this.vendorForm.get('zone')?.value;
    const divisionSub = this.vendorService.getDivisions(selectedZone).subscribe((data: any) => {
    this.divisions = data.filter((item: any) => item.zone==selectedZone)
      ...
      const submitSub = this.vendorService.submitVendorForm(formData).subscribe(response => {
        ...
        if (response && response.id) {
          ...
          const dialogRef = this.dialog.open(DialogConfirmationComponent, {
            ...
            nodal_email: this.vendorForm.get('nodal_person_email')?.value,
          type: 'vendor'  
